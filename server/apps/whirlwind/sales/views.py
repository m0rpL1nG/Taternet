...
class InvoiceViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = OrderDetailExt.objects.select_related('order_detail_id').select_related('invoice_number').select_related('vendor_invoice_id')
        ...
        if vendor_id:
            queryset = queryset.filter(order_detail_id__model_number__vendor_id=vendor_id)
        queryset = queryset.filter(invoice_detail_id__date__gte=datetime(2017, 03, 14, tzinfo=pytz.UTC), invoiced_yn=1)
